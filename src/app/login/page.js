...
import { Mail, Lock, Eye, EyeOff, Loader2, AlertCircle, ArrowRight, Activity } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Success flow: Pushes router to internal dashboard
      router.push("/dashboard");

    } catch (err) {
      console.error("Firebase Login error:", err);
      let customErr = "Invalid email or password. Please try again.";
      if (err.code === "auth/invalid-credential" || err.code === "auth/wrong-password" || err.code === "auth/user-not-found") {
        customErr = "Incorrect email address or password. Please verify your credentials.";
      } else if (err.code === "auth/too-many-requests") {
        customErr = "Account temporarily locked due to too many failed attempts. Please try again later.";
      } else if (err.code === "auth/invalid-email") {
        customErr = "The email address provided is invalid.";
      }
      setError(customErr);
    } finally {
      setLoading(false);
